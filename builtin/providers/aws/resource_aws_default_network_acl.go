...
func resourceAwsDefaultNetworkAclUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("subnet_ids") {
		...
		if len(remove) > 0 {
			...
			// association with one and the Default Network ACL. Because we're
			// managing the default here, we cannot do that, so we simply log a NO-OP.
			// In order to remove the Subnet, it must be destroyed, or assigned to a
			// different Network ACL
...
func resourceAwsDefaultNetworkAclDelete(d *schema.ResourceData, meta interface{}) error {
	log.Printf("[WARN] Cannot destroy Default Network ACL. Terraform will remove this resource from state as-is")
	d.SetId("")
	return nil
}
